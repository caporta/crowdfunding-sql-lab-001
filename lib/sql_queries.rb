...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS remaining_funds FROM projects JOIN pledges ON pledges.project_id = projects.id GROUP BY projects.title HAVING remaining_funds <= 0;"
end
